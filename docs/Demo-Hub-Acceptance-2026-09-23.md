...
4. Append the documented `demo` OAuth remote and `wing_demo` route to the desktop AgentPalace MCP configuration, preserving the existing remotes and routes. Restart the MCP connection. Call `agentpalace_search` with `wing: "wing_demo"`; on `authentication_required`, call `agentpalace_remote_auth_start` with `remote: "demo"`. With browser-capable desktop `auto` mode, confirm the system browser opens (or use the returned `authorization_url` if it does not) and complete consent as A; if the tool returns device mode, present its verification link and user code instead. Poll `agentpalace_remote_auth_status` until authenticated, then retry the MCP search. Write a drawer containing a unique test phrase to `wing_demo` with `agentpalace_add_drawer`, passing a deliberately false `added_by`. Search that phrase through the same remote; inspect the returned drawer/change metadata and record its creator and authenticated submitter IDs (opaque IDs only). Project mining is out of scope for this package ([#183](https://github.com/DigitumDei/agentpalace/issues/183)): the pinned engine has no `server.checkouts`, so `agentpalace mine` into `wing_demo` is rejected with HTTP 409 `checkout_unavailable`. MCP search only shows the attribution fields once [#180](https://github.com/DigitumDei/agentpalace/pull/180) is merged; before that, inspect the engine's stored record.
...
**Waived by the maintainer on 2026-09-25; not run.** There is no live evidence for either check.

- `compiled_gateway_acceptance` covers readonly B's refusal of KG, coordination and ingest writes: it asserts `403` for a readonly caller on each.
- No automated test sends an access-list `PUT` from a non-admin session, so B's and D's membership-edit refusals are not tested directly. `admin_gateway` covers them indirectly: it shows a demoted, non-admin session refused on `GET /hub/v1/access`. Every access-list handler, `GET` and `PUT` alike, goes through `admin_actor`, which refuses any role other than admin before checking CSRF or anything specific to the method. Separately, the findings above are open work: the client-side error reporting, installer and WSL issues need their own fixes. Whether they block launch is the maintainer's decision.

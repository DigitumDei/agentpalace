...
> **2026-09-24 annotation.** Live Google results for admin A, readonly B and writer D on desktop and WSL are recorded in [Live results — 2026-09-24](#live-results--2026-09-24). The unlisted account C was refused through device login; disabling B cut off its active grant; and a Compose restart kept the policy, content and attribution. C's browser login, B's readonly limit after re-enabling, D's delete/membership limits and a WSL sign-in after the restart are still pending, so the last row remains a launch blocker.
...
Account C (unlisted) refusal through browser login (device login is recorded above); confirming B is still limited to `readonly` after re-enabling and after the restart; D's delete and membership-edit rejection; and a WSL device sign-in after the restart. Real-Google acceptance stays **pending** until these are recorded.

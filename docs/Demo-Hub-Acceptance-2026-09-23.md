...
> **2026-09-24 annotation.** Live Google results for admin A, readonly B and writer D on desktop and WSL are recorded in [Live results — 2026-09-24](#live-results--2026-09-24). The unlisted account C was refused through device login; disabling B cut off its active grant. C's browser login, re-enabling B, D's delete/membership limits and the Compose restart checks are still pending, so the last row remains a launch blocker.
...
Account C (unlisted) refusal through browser login (device login is recorded above); re-enabling B and confirming a fresh sign-in works again; D's delete and membership-edit rejection; and the step 8 Compose restart, persistence and revocation checks. Real-Google acceptance stays **pending** until these are recorded.

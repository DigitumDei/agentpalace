...
| `POST /v1/coordination/messages` | `GET /v1/coordination/messages/{id}`; `GET /v1/coordination/inbox`; `GET /v1/coordination/events` | SQLite `coordination_messages`, `mutation_receipts`, and `coordination_events`; the message route populates `provenance_json`, while the other coordination sidecars remain reserved for future wiring | Preserve authenticated sender owner; retain sender/recipient agent identities and federated source separately; append acknowledger history | Owner-scoped `(idempotency_key, sender)`; recover message and event together before success | Legacy sender/recipient values remain visible; owner field is explicitly unknown |

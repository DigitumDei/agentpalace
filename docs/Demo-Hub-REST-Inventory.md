...
1. **Authenticated Owner Record:** The server associates each active token entry with an optional, validated, provider-neutral owner metadata record:
   ...
   ```
2. **Never Accept Owner from Request Body:** The client LLM or harness is never permitted to supply, override, or suggest `owner` fields. Existing production federation REST DTOs omit client-writable `owner` fields entirely, ensuring server-established ownership remains intact. Where mutation handlers or ingest paths process untyped or extended payloads, the `reject_payload_owner_claim` helper contract defines the server-side rejection mechanism to reject any caller-supplied owner claims.
...
| `Issuer` | 1–256 chars; printable ASCII (no whitespace or control chars); does not enforce URI syntax or reject sentinels | Transparent string (e.g. `"https://accounts.google.com"`). |
| `Subject` | 1–256 chars; printable ASCII (no whitespace or control chars) | Transparent string (e.g. `"104928190283019283019"`). Provider account identifier. |
| `SubjectBinding` | Holds `(Issuer, Subject)` | Structured JSON: `{ "issuer": "...", "subject": "..." }`. Binds provider issuer and subject. |
| `EmailAtWrite` | 3–254 chars; single `@` surrounded by non-empty local-part and domain (RFC 5321); no ASCII whitespace or control chars | Transparent string (e.g. `"tester@example.com"`). Advisory audit trace, non-authoritative. |
| `OwnerMetadata` | Validated bundle of `(OwnerId, Issuer, Subject, EmailAtWrite)` | Structured JSON object. Validates internal consistency. |
...
1. **Server-Assigned Provenance & Unauthenticated Owner Rejection Helper Contract:** Authenticated owner provenance is established exclusively by the server from token configuration (`OwnerMetadata` on `TokenEntry`) and carried into `AuthIdentity` / `AuthenticatedOwnerContext`. Existing production federation REST routes do not expose client-writable owner fields on request DTOs (`AddDrawerRequest`, `NewTaskRequest`, etc.), ensuring caller-asserted agent parameters (`added_by`, `sender`, etc.) cannot override or manufacture ownership. For mutation handlers and future storage-slice ingest processing untyped or extended payloads, the `reject_payload_owner_claim` helper contract provides an explicit check to reject caller-asserted owner fields with `ProvenanceError::UnauthenticatedOwnerClaim`, mapping to HTTP 400 Bad Request (`invalid_provenance`).

...
8. **Sealed Provenance Value Types & Persistence Boundary Validation:** `StorageOrigin` variant data and `OwnerScopedKey` components are sealed behind private fields, preventing unchecked external construction. All deserialization and envelope-building paths must use validated constructors. Fail-closed Serde parsing (`deny_unknown_fields`) rejects unknown or misspelled fields. Persistence boundaries enforce unbypassable validation (`validate(&self)` checked upon envelope serialization and deserialization).
